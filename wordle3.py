...
def gamePlay3(t):
    ...
    root = Tk()
    root.title("WORDLE")
    root.geometry("280x530")
    root.iconbitmap('3.ico')

    global word
    word = words.get_word3()
    print(word)
    ...
    btn.place(x=220, y=10, width=40, height=35)
                ...
                if len(guess) == 3:
                    if word == guess.lower():
                        for i, letter in enumerate(guess):
                            ...
                            label.grid(row=guessnum, column=i, padx=18, pady=13)
                        ...
                        for i, letter in enumerate(guess):
                            label = Label(f, text=letter.upper(), height=2, width=5)
                            label.grid(row=guessnum, column=i, padx=22, pady=13)
                    ...
                    messagebox.showerror("Character error", "Please use 3 characters in your guess")
                    var.set("")
                var.set("")
            else:
                print(guessnum)
    ...
    var = StringVar()
    wordInput = Entry(root, textvariable=var, width=22, font="vardana 12")
    wordInput.place(x=40, y=430)

    wordGuessButton = Button(root, text="Guess", font="Vardana 12", width=8, height=1, bg="light green",
                             command=getGuess)
    wordGuessButton.place(x=100, y=460)
    n = player_name.get()
    Label(text=f"Player name : {n}", font="Vardana 10", bg="#CECCBE").place(x=80, y=500)
